<template>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 294.5 294.5" width="25" height="25">
        <path d="M174.4,32.5c-6.8-10-12.8-19.2-18.4-27.6c-0.8-1.2-2-2.4-3.2-3.2c-4.8-3.2-11.2-1.6-14.4,3.2
			c-5.2,8.4-11.2,17.2-18.4,27.6C88.4,78.9,41.2,148.9,41.2,188.9c0,29.2,12,55.6,31.2,74.8c19.2,18.8,45.6,30.8,74.8,30.8
			s55.6-12,74.8-31.2s31.2-45.6,31.2-74.8C253.2,148.5,206,78.9,174.4,32.5z M207.6,248.9c-15.6,15.6-36.8,24.8-60.4,24.8
			s-44.8-9.6-60.4-24.8C71.2,233.3,62,212.1,62,188.5c0-33.6,45.2-100.4,75.2-144.8c3.6-5.2,6.8-10.4,10-14.8
			c3.2,4.4,6.4,9.6,10,14.8c30,44.8,75.2,111.2,75.2,144.8C232.4,212.1,222.8,233.3,207.6,248.9z"/>
        <path d="M209.2,181.3c-5.6-0.4-10.4,4-10.8,9.6c-0.4,8.8-3.2,17.2-7.6,24.4c-4.4,7.2-10.8,13.6-18.4,17.6c-4.8,2.8-6.8,9.2-4,14
			c3.2,5.6,9.6,7.2,14.4,4.4c10.8-6,19.6-14.8,25.6-24.8c6.4-10,10-22,10.4-34.4C219.2,186.5,214.8,181.7,209.2,181.3z"/>
    </svg></template>
<script>
  export default {
    name: "VoterIcon"
  }
</script>

<style scoped>

</style>