<template>
    <li class="scheduler-food__item" v-if="recipe">
        <div class="scheduler-food__item-wrap">
            ...
            <button  :class="['icon-button scheduler-food__btn-submenu',{'open':open}]"  class="" type="button" title="Открыть" @click="$emit('openItem')">
                <DownIcon/>
            </button>
            ...
            <div class="scheduler-food__meta">
                ...
                <p class="scheduler-food__ccal">{{ (recipe.weight/100)*recipe.calories }} Кал</p>
                <p class="scheduler-food__item-date">{{getTranslatedMealType(meal)}}: <b>{{showDateFormat(recipe.date)}}</b></p>
            </div>
        </div>
...
<script>
    ...
    export default {
        name: "RecipeItem",
        props:['recipeItem', 'open', 'meal','show'],
        data() {
            return {
                recipe: {}
            }
